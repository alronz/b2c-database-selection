In this example, I will show how we can model a complete B2C application data and perform more complex queries on it. I am going to use in this example the model used by the [TPC-H benchmark](http://www.tpc.org/tpch) which is shown below.
...
The idea behind this example is to show how we can use Cassandra to design a real B2C application data model and how to run complex SQL queries with joins, group by and order by clauses using CQL. I have chosen three queries from the TPCH benchmark that I think will cover most of the complex features of SQL. The chosen queries are q1, q3 and q4 as will be explained in the following sections.



The data used as input for this example is a small set of the data generated using TPCH DBGEN tool. The data is stored in CSV files corresponding to each data model object in the TPCH benchmark schema shown above (customer, supplier, part, lineitem, order , etc ...). For more details about how to generate the data, please have a look at [this](http://kejser.org/tpc-h-data-and-query-generation/) blog post. 

The complete code of this example can be found in Github along with the steps on how to run it. You can also change the input data used by changing the content of the [input files](https://github.com/alronz/B2C-Database-Selection-Implementations/tree/master/Cassandra/CassandraTPCHQueries/src/main/java/org/cassandra/tpcH/data) stored in the data folder then use the swagger UI to test the queries on different set of data.
...
As explained previously in the data layout section (link to there), writes are cheap in Cassandra and it is absolutely ok to use denormalisation for your data. We also model our data around the query patters that we are going to be performed in our application and not around the entities of the application. This means that if we are going to frequently run the above query in our application, then we should create a separate table for it that satisfy the above query and scale efficiently at the same time. 

Ofcourse first we will create the Keyspace that will be used in the whole example as seen below:
...
The important thing that you need to consider carefully before creating a CQL table is the primary key. The primary key can be a single key, a composite key or compound key and based on the primary key the data will be written to the partitions. If you use a primary key that is not unique, the data will be overwritten. This means you should always ensure that the primary key is unique to avoid overwriting your data. The primary key contains also the partition key which could be a single key or a composite key. The partition key defines how the data will be distributed across the different nodes, which means we should choose partition keys that can be distributed equally across the nodes and not creating what is called a hot spot in one of the partitions. A hot spot is created when the data is distributed unequally among the partitions where one of the partitions contains most of the data. Additionally, Cassandra support only two billion columns per partition and after this limit it will reject the write requests. If you also choose a partition key that is too high,  then each partition will have little data which will negate the benefits of the wide row data model where you will have too little data in each partition that worth ordering. For all these reasons, it is quite important that we choose a unique primary key to prevent data from overwriting and a good partition key that is not too high or too low and will allow for a good data distribution across the cluster. 

From the SQL query, we have to perform aggregate functions such as sum, avg and count on the data inside the lineitem table grouped by the returnflag, and linestatus. In Cassandra aggregates are always performed in partition level, this means that the data will always be grouped by the partition key. Hence we will have to make sure that the linestatus and returnflag are part of the composite key that will make the partition key since they are needed to perform the aggregate functions. However if we use only returnflag and linestatus as primary key, then the data will be overwritten in each partition since they are not unique. For example, we could have multiple rows in the lineitem table having the same linestatus and returnflag. So we added the orderkey and the linenumber to the compound key as clustering columns which will make the primary key unique and will prevent overwriting the data. The primary key is ((returnflag,linestatus),orderkey,linenumber). But then since we want to filter results using the shipmate, then a good idea is to include it in the compound key. So now our compound key is  ((returnflag,linestatus),shipdate,orderkey,linenumber). The shipmate should be the first clustering column since we want to run range queries using it. If you put the shipdate as the second clustering column, then to run range queries on it you need first to specify the value of the first clustering column using either '=' or 'IN' and not inequality otherwise you will get an error.  Note that the partition key is (returnflag,linestatus) which is good somehow for data distribution where each partition will have the data of a specific returnflag and linestatus.
